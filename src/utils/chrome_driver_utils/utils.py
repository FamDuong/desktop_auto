...
def get_coccoc_version():
    ...
    if platform == 'linux':
        path = get_linux_executable_path()
        with subprocess.Popen([path, '--version'], stdout=subprocess.PIPE) as proc:
            version = proc.stdout.read().decode('utf-8').replace('Chromium', '').replace('CocCoc', '').strip()
    elif platform == 'mac':
        process = subprocess.Popen(['/Applications/CocCoc.app/Contents/MacOS/CocCoc', '--version'],
                                   stdout=subprocess.PIPE)
        version = process.communicate()[0].decode('UTF-8').replace('CocCoc', '').strip()
    elif platform == 'win':
        process = subprocess.Popen(
            ['reg', 'query', 'HKEY_CURRENT_USER\\Software\\CocCoc\\Browser\\BLBeacon', '/v', 'version'],
            stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, stdin=subprocess.DEVNULL
        )
        output = process.communicate()
        if output and output[0] and len(output[0]) > 0:
...
def download_chromedriver(path: Optional[AnyStr] = None, no_ssl: bool = False, browser_type='CocCoc'):
    """
    ...
    """
    if browser_type == 'CocCoc':
        chrome_version = get_coccoc_version()
    else:
