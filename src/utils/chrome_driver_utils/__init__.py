...
from . import utils
from .. import os_utils, file_utils


def install(cwd: bool = True, path: Optional[AnyStr] = None, no_ssl: bool = False, browser_type='CocCoc'):
    """
    ...
    Args:
        browser_type: CocCoc default, CHROME also
    """
    if cwd:
        # path = os.getcwd() + r'\drivers'
        if not file_utils.check_folder_is_exists(rf'C:\Users\{os_utils.get_username()}\.wdm\drivers\chromedriver\win32'):
            os.makedirs(rf'C:\Users\{os_utils.get_username()}\.wdm\drivers\chromedriver\win32')
        path = rf'C:\Users\{os_utils.get_username()}\.wdm\drivers\chromedriver\win32'
    chromedriver_filepath = utils.download_chromedriver(path, no_ssl, browser_type)
